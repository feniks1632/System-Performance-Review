...
class TestRespondentFunctionalityFinal:
    ...
    def test_respondent_can_access_assigned_goals(
        ...
        assert employee_login.status_code == 200, f"Employee login failed: {employee_login.text}"
        ...
        assert login_response.status_code == 200, f"Respondent login failed: {login_response.text}"
        respondent_headers = {"Authorization": f"Bearer {login_response.json()['access_token']}"}

        # Респондент получает свои цели
        response = client.get(
            "/api/v1/goals/respondent/my",
            headers=respondent_headers
        )
    ...
    def test_respondent_can_get_specific_goal(
        ...
        response = client.get(
            f"/api/v1/goals/respondent/{goal_id}",
            headers=respondent_headers
        )
    ...
    def test_respondent_can_get_goal_steps(
        ...
        response = client.get(
            f"/api/v1/respondent/{goal_id}/steps",
            headers=respondent_headers
        )
    ...
    def test_respondent_can_use_regular_endpoints(
        ...
        login_response = client.post(
            "/api/v1/auth/login",
            json={
                "email": "respondent_regular@test.com",  # ИСПРАВЛЕННЫЙ EMAIL
                "password": "password123"
            },
        )
        assert login_response.status_code == 200, f"Respondent login failed: {login_response.text}"
        ...
        response = client.get(
            f"/api/v1/goal-steps/goals/{goal_id}/steps",
            headers=respondent_headers
        )
        
        assert response.status_code == 200

    def test_stranger_cannot_access_goal(
        self, client, test_employee_user, db_session
    ):
        ...
        stranger_response = client.post("/api/v1/auth/register", json=stranger_registration)
        ...
        assert login_response.status_code == 200, f"Stranger login failed: {login_response.text}"
        stranger_headers = {"Authorization": f"Bearer {login_response.json()['access_token']}"}
...
def test_respondent_workflow_fixed(client, test_employee_user, db_session):
    ...
    response = client.get(f"/api/v1/goals/respondent/{goal_id}", headers=respondent_headers)
    assert response.status_code == 200
    goal_data = response.json()
    assert goal_data["title"] == "Workflow цель исправленная"

    # 5.3 Получаем подпункты через специальный эндпоинт
    response = client.get(f"/api/v1/respondent/{goal_id}/steps", headers=respondent_headers)
    assert response.status_code == 200
    steps = response.json()
    assert len(steps) == 2
