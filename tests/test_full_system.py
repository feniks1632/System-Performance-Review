...
def test_full_system_workflow(
    ...
    logger.info("- Сотрудник: %s (%s)", test_employee_user.full_name, test_employee_user.email)
    logger.info("- Руководитель: %s (%s)", test_manager_user_complete.full_name, test_manager_user_complete.email)
    ...
    goals_response = client.get(
        f"/api/v1/goals/employee/{test_employee_user.id}", 
        headers=employee_auth_headers
    )
    assert goals_response.status_code == 200, f"Ошибка получения целей: {goals_response.text}"
    ...
    assert questions_response.status_code == 200, f"Ошибка получения вопросов: {questions_response.text}"
    ...
    if questions:
        ...
        self_review_response = client.post(
            "/api/v1/reviews/", 
            json=self_review_data, 
            headers=employee_auth_headers
        )
        assert self_review_response.status_code == 200, f"Ошибка создания самооценки: {self_review_response.text}"
    ...
    assert manager_questions_response.status_code == 200, f"Ошибка получения вопросов руководителя: {manager_questions_response.text}"
    ...
    if manager_questions:
        ...
        assert manager_review_response.status_code == 200, f"Ошибка создания оценки руководителя: {manager_review_response.text}"
        manager_review = manager_review_response.json()
        logger.info("Оценка руководителя создана, балл: %.2f", manager_review['calculated_score'])

    # 6. Тестируем аналитику
    logger.info("6. Тестируем аналитику")
    
    analytics_response = client.get(
        f"/api/v1/analytics/goal/{goal_id}", 
        headers=manager_auth_headers
    )
    assert analytics_response.status_code == 200, f"Ошибка получения аналитики: {analytics_response.text}"
    ...
    assert summary_response.status_code == 200, f"Ошибка получения сводной аналитики: {summary_response.text}"
