...
class TestGoalsIntegration:
    def test_create_goal_with_steps(
        self, client, auth_headers, db_session, test_goal_data
    ):
        ...
        user = (
            db_session.query(User).filter(User.email.like("test_%@example.com")).first()
        )
        if not user:
    ...
    def test_update_goal_status(self, client, auth_headers, test_goal_data):
        ...
        ), f"Status update failed: {status_response.text}"
    ...
    def test_goal_step_operations(self, client, auth_headers, test_goal_data):
        ...
        step_response = client.post(
            f"/api/v1/goals/{goal_id}/steps", json=step_data, headers=auth_headers
        )
        assert step_response.status_code == 200
        step_id = step_response.json()["id"]

        # Обновляем подпункт
        update_data = {"title": "Updated Step", "is_completed": True}
        update_response = client.put(
            f"/api/v1/steps/{step_id}", json=update_data, headers=auth_headers
        )
        ...
        delete_response = client.delete(
            f"/api/v1/steps/{step_id}", headers=auth_headers
        )
        assert delete_response.status_code == 200

        # Проверяем что подпункт удален
        steps_response = client.get(
            f"/api/v1/goals/{goal_id}/steps", headers=auth_headers
        )
